...
const e11000Error = (err: any): TGenericErrorResponse => {
  const duplicateKeyErrorDetails = err.message.match(
    /E11000 duplicate key error collection: (\S+) index: (\S+) dup key: (.+)/,
  );
  const regex = /"([^"]+?)"/;
